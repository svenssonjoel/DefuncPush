...
pushLength (Select b p1 p2) = pushLength p1 -- hack
...
apply (Generate ixf n) = \k -> par_ n $ \i ->
                           applyW k i (ixf i)

apply (Use mem l) = \k -> par_ l $ \i ->
                            do a <- read mem i
                               applyW k i a 
...
compileSimple2' = runCM 0 $ toVector $ takeSome (simple2 input11 :: PushT (Expr Int)) 10
